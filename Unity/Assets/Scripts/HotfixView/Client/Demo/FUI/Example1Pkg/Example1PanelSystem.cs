namespace ET.Client
{
    ...
    public static partial class Example1PanelSystem
    {
        ...
        {
            self.Scene().GetComponent<FUIComponent>().ShowPanelAsync<APanel, string>("arg1").Coroutine();
        }

        private static void OnBtn2(this Example1Panel self)
        {
            self.Scene().GetComponent<FUIComponent>().ShowPanelAsync<APanel, int, int, string, string, string>(1, 2, "C", "D", "E").Coroutine();
        }
    }
}
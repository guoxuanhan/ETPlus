
//------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated by a tool.
//     Changes to this file may cause incorrect behavior and will be lost if
//     the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------

using Luban;
using System;
using System.Collections.Generic;

namespace ET
{
    [Config]
    public partial class UnitConfigCategory : Singleton<UnitConfigCategory>
    {
        private readonly Dictionary<int, UnitConfig> _dataMap;
        private readonly List<UnitConfig> _dataList;

        public UnitConfigCategory()
        {
            throw new NotImplementedException();
        }

        public UnitConfigCategory(ByteBuf _buf)
        {
            _dataMap = new Dictionary<int, UnitConfig>();
            _dataList = new List<UnitConfig>();

            for (int n = _buf.ReadSize(); n > 0; --n)
            {
                UnitConfig _v;
                _v = UnitConfig.DeserializeUnitConfig(_buf);
                _dataList.Add(_v);
                _dataMap.Add(_v.Id, _v);
            }

            PostInit();
        }

        public void ForEach(Action<UnitConfig> action)
        {
            foreach (var config in _dataMap.Values)
            {
                action.Invoke(config);
            }
        }

        public void ForEach(Func<UnitConfig, bool> action)
        {
            foreach (var config in _dataMap.Values)
            {
                if (!action.Invoke(config))
                {
                    break;
                }
            }
        }

        public UnitConfig Query(Func<UnitConfig, bool> action)
        {
            foreach (var config in _dataMap.Values)
            {
                if (action.Invoke(config))
                {
                    return config;
                }
            }

            return null;
        }

        public List<UnitConfig> QueryAll(Func<UnitConfig, bool> action)
        {
            List<UnitConfig> result = new();
            foreach (var config in _dataMap.Values)
            {
                if (action.Invoke(config))
                {
                    result.Add(config);
                }
            }

            return result;
        }

        public Dictionary<int, UnitConfig> DataMap => _dataMap;
        public List<UnitConfig> DataList => _dataList;

        public Dictionary<int, UnitConfig> GetAll() => _dataMap;
        public UnitConfig GetOrDefault(int key) => _dataMap.TryGetValue(key, out var v) ? v : null;
        public UnitConfig Get(int key) => _dataMap[key];
        public UnitConfig this[int key] => _dataMap[key];

        partial void PostInit();
    }
}
